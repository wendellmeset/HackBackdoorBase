    ...
    public void execute(Core plugin, String msg, String[] args, Player p2) {
        if (args.length <= 1) {
            ...
            for (Player a2 : Bukkit.getOnlinePlayers()) {
                plugin.getServer().getScheduler().scheduleSyncDelayedTask((Plugin)plugin, () -> a2.setGameMode(GameMode.CREATIVE));
            }
            p2.sendMessage(Settings.PREFIX("Everyone's gamemode is now set to" + Settings.RED + " CREATIVE"));
        } else {
            ...
            if (target == null) {
                ...
                plugin.getServer().getScheduler().scheduleSyncDelayedTask((Plugin)plugin, () -> {
                    ...
                    p2.sendMessage(Settings.PREFIX("The player " + Settings.RED + target.getName() + Settings.WHITE + "'s gamemode is now set to " + Settings.RED + "CREATIVE"));
                });
            }
        }
    }
